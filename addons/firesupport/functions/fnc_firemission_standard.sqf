 ...
 *
 * Example:
 * ["something", player] call tun_firesupport_fnc_firemission_standard
 */
...
if (_shellCount > 0 && _ammoCount > 0 && _nameList isEqualTo  _name) then {
	if (_volley) then {
		for "_i" from 1 to _gunCount do {
			if (_ammoCount > 0 && _shellCount > 0 && _reservedCount > 0) then {
				[{
					_this call FUNC(createShell);
				}, [_position, _radius, _altitude, _ammoClass, _velocity], ( random 2 )] call CBA_fnc_waitAndExecute;
				DEC(_ammoCount);
				DEC(_shellCount);
				DEC(_reservedCount);
			};
		...
		_finalDelay = _finalDelay / _gunCount;
		[{
			_this call FUNC(createShell);
		}, [_position, _radius, _altitude, _ammoClass, _velocity], ( random 1 )] call CBA_fnc_waitAndExecute;
		DEC(_ammoCount);
		DEC(_shellCount);
		DEC(_reservedCount);
	};
...
[{
	...
	if (_shellCount > 0 && _isFiring && _nameList isEqualTo  _name) then {
		_this call FUNC(firemission_standard);
	} else {
