...
[
    QGVAR(required_items), // Unique setting name. Matches resulting variable name <STRING>
    "EDITBOX", // Type of setting. Can be "CHECKBOX", "EDITBOX", "LIST", "SLIDER" or "COLOR" <STRING>
    [localize "STR_tun_firesupport_cba_required_items_classnames", localize "STR_tun_firesupport_cba_required_items_classnames_tooltip"], // Display name or display name + tooltip (optional, default: same as setting name) <STRING, ARRAY>
    localize "STR_tun_firesupport_cba_Category_main", // Category for the settings menu + optional sub-category <STRING, ARRAY>
   	"ItemGPS,ace_microdagr", // Extra properties of the setting depending of _settingType.
    1, // 1: all clients share the same setting, 2: setting can't be overwritten (optional, default: 0) <ARRAY>
    {GVAR(required_items) = GVAR(required_items) splitString ",";}, // Script to execute when setting is changed. (optional) <CODE>
    true //Setting will be marked as needing mission restart after being changed. (optional, default false) <BOOL>
] call CBA_Settings_fnc_init;
